 ...
 ***********************************************************************************************
 *
 * $Log:: /JP2_PC/Source/Lib/Renderer/Primitives/P5/DrawSubTriangleEx.cpp                      $
 * 
 * 21    8/27/98 9:13p Asouth
 * 'fixed' class explicitly scoped
 * 
 * 20    97.12.22 9:08p Mmouni
 * Fixed error with destination of terrain gouraud shading primitive.
 * 
 * 19    97.12.03 8:43p Mmouni
 * Removed self modification from non-transparent copy primitives.
 * 
...
INNER_LOOP_2P:
		...
		shl		eax,16								// Shift first pixel up.
		add		esi,ebx								// Step U fraction.
		...
		mov		ebx,[pvBaseOfLine]					// Pointer to base of scanline.
		mov		al,[edx*2]							// Read texture value (low).

		mov		ebp,[w2dDeltaTex + ebp*4 + 4]CWalk2D.iUVInt		// Integer step based on V carry.
		mov		ah,[edx*2 + 1]									// Read texture value (high).

		adc		edx,ebp							// Integer step + U carry.
			
		test	eax,eax
		jz		short INNER_LOOP_2P				// Both transparent in this case.

		test	eax,0xffff0000
		jz		short PIXEL1_TRANS_PIXEL2_SOLID
		...
		//
		rol		eax,16								// Reverse pixels.

		mov		[ebx + edi*2 - 4],eax				// Store both pixels.
		jmp		INNER_LOOP_2P
...
PIXEL1_TRANS_PIXEL2_SOLID:
		...
		//
		mov		[ebx + edi*2 - 2],ax				// Store second pixel.
...
PIXEL1_SOLID_PIXEL2_TRANS:
		...
		mov		[ebx + edi*2 - 4],ax				// Store first pixel.
...
INNER_LOOP_2P:
		mov		ebx,[w2dDeltaTex]CWalk2D.uVFrac		// Load V fraction step.
		mov		ah,[edx*2]							// Read texture value (low).

		add		ecx,ebx								// Step V fraction.
		mov		al,[edx*2 + 1]						// Read texture value (high).
		...
		mov		ebp,[w2dDeltaTex + ebp*4 + 4]CWalk2D.iUVInt		// Integer step + U carry.
		mov		al,[edx*2 + 1]									// Read texture value (high).
		
		bswap	eax									// Reverse pixels (perserve falgs).
		
		adc		edx,ebp								// Integer step + U carry.
		mov		[ebx + edi*2 - 4],eax				// Store pixel.
...
LIN_NEXT_SCAN_LINE:
		...
		mov		ebx,[pvTextureBitmap]									// Texture base.
		mov		[pvBaseOfLine],ebp										// Save base of line.

		shr		ebx,1													// Convert to word pointer.
		lea		ebp,[ebp + edi*2]										// Calculate destination address.
		...
		mov		[ebx + edi*2 - 2],ax				// Store pixel.
