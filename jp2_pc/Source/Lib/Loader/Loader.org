...
CProfileStat psBumpPack("Bumpmap pack", &psLoader);
	...
	CLoadWorld::CLoadWorld(const char* str_filename, PFNWORLDLOADNOTIFY pfnWorldNotify, uint32 ui32_NotifyParam) 
	{
		...
		padAudioDaemon->LoadForGRF(str_filename);
        ...
        if (pfnWorldNotify)
        {
            (pfnWorldNotify)(ui32_NotifyParam, 1, icObjects, goiInfo.uObjectCount);
        }
...
/*		uint32 u4_test;
		dprintf("%x\n",u4_test = u4Hash(gon.strObjectName,0,true));
		if (u4_test == 0x01eb9bee)
		{
			_asm int 3;
		}*/
		...
		if (ac_platonic_ideal[0] != '\0')
		{
			...
			if (i != tmPlatonicIdeal.end())
			{
				...
				if (pins)
				{
					...
					pins->SetPresence(::pr3Presence(gon));
