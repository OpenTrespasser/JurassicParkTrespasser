...
bool CVideoWnd::ServiceAudioBuffer(bool force)
{
    ...
    if (m_audioWriteFinished) {
        if (playOffset > m_audioState.m_finalWriteOffset && writeOffset > m_audioState.m_finalWriteOffset) {
            m_soundbuffer->Stop();
...
BOOL CVideoWnd::Play(LPCSTR pszFile)
{
    ...
    while (!m_fVideoOver)
    {
        ...
        while (std::chrono::high_resolution_clock::now() - start < std::chrono::microseconds(static_cast<long>(usf)))
            std::this_thread::sleep_for(0ms);
        if (m_fVideoLoaded && m_frameQueue.empty() && m_audioState.m_audioFinished)
            m_fVideoOver = true;
    }
