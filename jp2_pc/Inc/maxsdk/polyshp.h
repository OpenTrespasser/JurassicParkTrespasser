...
class PolyPt {
	public:
		...
		PolyPt() { p = Point3(0,0,0); flags = 0; aux = 0; }
		PolyPt(Point3 ip, DWORD f = 0, int a=0) { p = ip; flags = f; aux = a; }
	};
...
class PolyLine {
	public:
		...
		BOOL IsClosed() { return (flags & POLYLINE_CLOSED) ? TRUE : FALSE; }
		void Open() { flags &= ~POLYLINE_CLOSED; }
		BOOL IsOpen() { return (flags & POLYLINE_CLOSED) ? FALSE : TRUE; }
		...
		int Segments() { return numPts - (IsClosed() ? 0 : 1); }
		...
		CoreExport void Render(GraphicsWindow *gw, BOOL colorSegs, BitArray &segsel);
		...
		CoreExport BOOL HitsSegment(Point2 p1, Point2 p2);
